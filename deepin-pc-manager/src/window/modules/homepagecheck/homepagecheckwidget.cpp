...
HomePageCheckWidget::HomePageCheckWidget(HomePageModel *model, DFrame *parent)
    : DFrame(parent)
    , m_guiHelper(nullptr)
    , m_model(model)
    , m_lbScore(nullptr)
...
void HomePageCheckWidget::recSSHStatus(bool opened)
{
    ...
    if (CheckStatus::Checking == m_statusSSH) {
        ...
        if (opened) {
            ...
            m_sProblemsInfo += tr("Remote access (SSH)");
    ...
    setCheckItemSpinnerByStatus(CheckItemIndex::SSH, settingStatus);
    // 添加日志
    m_model->addSecurityLog(tr("Remote access (SSH): OFF"));
...
bool HomePageCheckWidget::eventFilter(QObject *obj, QEvent *event)
{
    if (obj == this) {
        // 如果是字体改变事件，则要求改变"修复"按钮文案显示
        if (event->type() == QEvent::FontChange) {
            refreshRepairBtnTextChange();
        }
    }
    return QObject::eventFilter(obj, event);
}

void HomePageCheckWidget::refreshRepairBtnTextChange()
{
    QFontMetrics fontMet(m_btnRepair->font());
    QString originalStr = tr("Fix All");
    QString elideStr = fontMet.elidedText(originalStr, Qt::ElideMiddle, m_btnRepair->width() - 10);
    m_btnRepair->setText(elideStr);
    if ((m_btnRepair->width() - 10) < fontMet.width(originalStr)) {
        m_btnRepair->setToolTip(originalStr);
    } else {
        m_btnRepair->setToolTip("");
    }
}
...
void HomePageCheckWidget::initUI()
{
    ...
    // 设置字体
    pointLabel->setFont(Utils::getFixFontSize(17));
    // 添加到布局中
    ...
    m_btnAutoStartIgnoredCancel->setFont(Utils::getFixFontSize(14));
    ...
    m_btnSSH->setFont(Utils::getFixFontSize(14));
    ...
    m_btnSystemVersion->setFont(Utils::getFixFontSize(14));
    ...
    m_btnCleaner->setFont(Utils::getFixFontSize(14));
    ...
    m_btnDiskCheck->setFont(Utils::getFixFontSize(14));
    ...
    m_btnRootModelIgnored->setFont(Utils::getFixFontSize(14));
    ...
    // 取消按钮
    m_btnCheckCancel = new DPushButton(tr("Cancel", "button"), this);
    ...
    m_btnCheckCancel->setFont(Utils::getFixFontSize(14));

    // 加入控件到布局
    layoutBottomBtn->addWidget(m_btnCheckCancel);

    // 返回按钮
    m_btnGoBack = new DPushButton(tr("Back", "button"), this);
    ...
    m_btnGoBack->setFont(Utils::getFixFontSize(14));
    ...
    m_btnRecheck->setFont(Utils::getFixFontSize(14));

    // 加入控件到布局
    layoutBottomBtn->addWidget(m_btnRecheck);
    // 完成按钮
    m_btnFinish = new DSuggestButton(tr("Done", "button"), this);
    ...
    m_btnFinish->setFont(Utils::getFixFontSize(14));
    // 加入控件到布局
    ...
    m_btnRepair->setFont(Utils::getFixFontSize(14));

    refreshRepairBtnTextChange(); // 刷新按钮文案显示
    // 加入控件到布局
...
void HomePageCheckWidget::initCheckItemPicAndName()
{
    ...
    // 设置 ssh 标题
    m_lbSSHTitle->setText(tr("Remote access (SSH)"));
